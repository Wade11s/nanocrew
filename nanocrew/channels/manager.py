...
class ChannelManager:
    ...
    def _init_channels(self) -> None:
        ...
        if self.config.channels.telegram.enabled:
            try:
                from nanocrew.channels.telegram import TelegramChannel
        ...
        if self.config.channels.discord.enabled:
            try:
                from nanocrew.channels.discord import DiscordChannel

                self.channels["discord"] = DiscordChannel(
                    self.config.channels.discord, self.bus, self.agent_manager
                )
        ...
        if self.config.channels.feishu.enabled:
            try:
                from nanocrew.channels.feishu import FeishuChannel
        ...
        if self.config.channels.mochat.enabled:
            try:
                from nanocrew.channels.mochat import MochatChannel

                self.channels["mochat"] = MochatChannel(
                    self.config.channels.mochat, self.bus, self.agent_manager
                )
        ...
        if self.config.channels.dingtalk.enabled:
            try:
                from nanocrew.channels.dingtalk import DingTalkChannel

                self.channels["dingtalk"] = DingTalkChannel(
                    self.config.channels.dingtalk, self.bus, self.agent_manager
                )
        ...
        if self.config.channels.email.enabled:
            try:
                from nanocrew.channels.email import EmailChannel

                self.channels["email"] = EmailChannel(
                    self.config.channels.email, self.bus, self.agent_manager
                )
        ...
        if self.config.channels.slack.enabled:
            try:
                from nanocrew.channels.slack import SlackChannel

                self.channels["slack"] = SlackChannel(
                    self.config.channels.slack, self.bus, self.agent_manager
                )
        ...
        if self.config.channels.qq.enabled:
            try:
                from nanocrew.channels.qq import QQChannel
        ...
        while True:
            try:
                ...
                try:
                    ...
                    logger.debug(
                        f"Routing message from {session_key} to agent '{agent_loop.context.workspace}'"
                    )
